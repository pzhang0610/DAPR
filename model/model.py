...
class SDAN(nn.Module):
    def __init__(self, num_strips, num_features, num_classes, drop=0):
        ...
        self.joint_seg = JointSegConv(in_channel=2048, num_features=self.num_features, num_classes=self.num_classes, num_strips=self.num_strips, bnneck=True)
    ...
    def forward(self, rgb_img, ir_img, alpha, out_feature=False, norm=False):
        ...
        rgb_feat = self.conv_rd(rgb_feat)#
        ir_feat = self.conv_rd(ir_feat)#
